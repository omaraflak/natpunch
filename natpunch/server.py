            ...
            while True:
                ...
                if message.message_id == Message.MESSAGE_JOIN:
                    ...
                    if uid in self.rooms:
                        ...
                        del self.rooms[uid]
                        time = int(
                            (datetime.now() + timedelta(seconds=self.connect_delay)).timestamp())
                        MessageConnect(ip1, port1, ip2, port2,
                                       time).to_message().send(client1)
                        MessageConnect(ip2, port2, ip1, port1,
                                       time).to_message().send(client2)
                    else:
                        self.rooms[uid] = client1
                        self._call_later(lambda: self._remove_room_and_notify(
                            uid, client1), self.room_ttl)
        except socket.error as e:
            logging.error(e)
            return
        finally:
